...
const dataCases: Array<[string, string]> = [
  ['all types json', fs.readFileSync(path.resolve(__dirname, './fixtures/all_types_json.txt'), 'utf8')],
  ['simple json data', fs.readFileSync(path.resolve(__dirname, './fixtures/simple_json.txt'), 'utf8')],
  ['merges nested arrays', fs.readFileSync(path.resolve(__dirname, './fixtures/merges_nested_arrays.txt'), 'utf8')],
];
...
const settingCases = versions
  .map((version) => {
    const value: Array<[string, Settings]> = [
      ...
      [
        version +
          ' with getters, fluent setters, all docblocks, extra new lines for properties, from json method and to array method',
        {
          phpVersion: version,
...
const cases = dataCases
  .map((dataCase) => {
    return settingCases.map((settingCase) => {
      return [dataCase[0] + ' ' + settingCase[0], dataCase[1], settingCase[1]];
    });
  })
