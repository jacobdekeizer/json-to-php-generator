...
export default class PhpClassFromJsonMethodPresenter {
  ...
  public write(codeWriter: CodeWriter): void {
    ...
    const arrayPresenter = new PhpPropertyTypePresenter(new PhpProperty(this.paramName).add(paramType), this.settings);

    new PhpDocblockPresenter(this.settings, [arrayPresenter], 'self').write(codeWriter);

    codeWriter.openMethod(PhpVisibility.Public, 'fromJson', 'self', [`${paramType.getType()} ${this.paramVar}`], {
      isStatic: true,
    });
    ...
    if (this.settings.addConstructor) {
      ...
      this.writeWithoutConstructor(codeWriter, (type) => type.getPhpVarName() + ' = ', ';');
    }
  ...
  private getPropertyFromData(typePresenter: PhpPropertyTypePresenter): string[] {
    ...
    const classArrayType = property.getTypes().find((type) => type instanceof ArrayType && type.isPhpClassArray()) as
      | ArrayType
      | undefined;
    ...
    if (classArrayType) {
      ...
      lines.push(`${property.isNullable() ? `${dataItemNullCheck} ? ` : ''}array_map(static function($data) {`);
